...
public class AdministrationManage extends HttpServlet{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(mop==4){
            response.sendRedirect("./systemManagement/addMember.jsp?mop=4");
...
//                    "&remarks="+remarks
            ...
            response.sendRedirect("./systemManagement/editMember.jsp?mop=5");
        }else if(mop==6){
            response.sendRedirect("./systemManagement/waiterUpdate.jsp?mop=6");
        }else if(mop==7){
            response.sendRedirect("./systemManagement/showMembers.jsp?mop=7");
